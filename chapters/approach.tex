...
There is a C++ based version in the git-repository of SPIRAL which is also fitted with
...
Other honorable mentions are painting programs such as \todo{these two weird software stuff things}
...
value of 20 time steps per stroke is picked \todo{this is actually different as the steps depend on the lenght of the path. Look this up}.
...
This vectorized version of alpha blending will introduce a new possible bottleneck
...
As an evolutionary step to perceptual loss, Zhang \etal \cite{lpips} introduced 
...
\paragraph{Handpicked losses} As the data distribution for the generated data set
...
Now, the axes of the original multivariate distribution co-align with $\vec{c}^\parallel(x)$
and $\vec{c}^\bot(x)$.
...
As it has been explained in \refsec{opt:partialupdates}, it is not possible to optimize
...
One choice, which has to be made individually per target image is how many brush strokes
...
Results can be seen in \reffig{opt:results:starry_night} for Starry Night as the standard reference
