...
They would formulate an energy function to describe how well the parameters describe the image~\cite{hertzmannreview}.
...
All in all, fluidpaint has been chosen as the tool to generate the data set.
...
\begin{marginfigure}
    \subfloat[Original Brushstrokes]{%
        \includegraphics{brushstrokes_vg}
    }\qquad
    \subfloat[Sample Brushstroke]{%
        \includegraphics{zolee}
    }\qquad
...
\begin{marginfigure}
    \includegraphics{starry_strokes}
...
\end{align}
\reffig{scatter} shows samples from this distribution for an exemplary set of start and end point.
\begin{marginfigure}
    \resizebox{\textwidth}{!}{
...
\begin{figure*}
    ...
    \resizebox{1.5\textwidth}{!}{
        ...
        \caption{Visualization of the generator and discriminator architectures. This schematic does not include stylemod, noise or multi-scale inputs to allow for a better overview.}\labfig{genarch}
        \labfig{architecture}
\end{figure*}
...
Especially, since a painting image ($1000\times1000 \si{\pixel}$) is several times larger than the render window ($64\times64$).\\
...
For Gaussian distributions the KL divergence is well defined~\cite[p.~76]{deeplearning}}
