...
\end{marginfigure}
Doing so limits the control over each drop of paint but maintains enough control to still create very delicate details in paintings.
...
Quite similarly, the objective of an optimizer is to replicate an image by only using brushstrokes (which may overlap).
...
Nonetheless, it is theoretically possible to create a differentiable renderer \cite{worldmodel}.
...
The basic idea was proposed by \citeauthor*{neuralpainters}~\cite{neuralpainters} as it facilitates training of reinforcement learning based networks.
...
The neural renderer in \citeauthor*{neuralpainters}'s~\cite{neuralpainters} work improves an architecture based on SPIRAL \cite{SPIRAL}.
