...
Due to the often observed change in appearance in many artistic images (\eg abstraction), existing CV pipelines for \ie classification usually do not work as intended.
...
Another example is art created with the help of neural networks \cite{GANart}.
Research in the artistic field also incorporates techniques from computer vision and natural sciences to aid the detection of forgeries \cite{picassomatissefake} \cite{guardianarticle}.
...
Drawing network often rely on recurrent architectures (see \refsec{RNN}) or iterative approaches where the current state of the drawing is taken into account along the target-image.
